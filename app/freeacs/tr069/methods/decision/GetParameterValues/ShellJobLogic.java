...
import freeacs.dbi.Job;
import freeacs.dbi.ScriptExecutions;
...
import freeacs.tr069.base.UnitJob;
import freeacs.tr069.exception.TR069DatabaseException;
...
public class ShellJobLogic {
     ...
     */
    private static void executeShellScript(SessionData sessionData, freeacs.dbi.Job job, UnitJob uj, boolean discovery, freeacs.dbi.ScriptExecutions execs)
            throws TR069Exception {
        String scriptArgs =
                "\"-uut:"
        ...
        while (true) {
            try {
                ...
                timeWaited += timeWait;
                timeWaitFactor += 2;
                freeacs.dbi.ScriptExecution se = execs.getExecution(sessionData.getUnittype(), requestId);
                if (se.getExitStatus() != null) {
                    if (se.getExitStatus()) { // ERROR OCCURRED
                        log.error(se.getErrorMessage());
                        uj.stop(freeacs.dbi.UnitJobStatus.CONFIRMED_FAILED, discovery);
                    } else uj.stop(freeacs.dbi.UnitJobStatus.COMPLETED_OK, discovery);
                    break;
                }
                if (timeWaited > 30000) {
    ...
    private static void toCPE(SessionData sessionData, DBI dbi) throws TR069DatabaseException {
        freeacs.dbi.UnittypeParameters utps = sessionData.getUnittype().getUnittypeParameters();
        freeacs.dbi.Unit unit;
        try {
        ...
        for (int i = 0; i < sessionData.getValuesFromCPE().size(); i++) {
            ...
            freeacs.dbi.UnittypeParameter utp = utps.getByName(pvsCPE.getName());
            if (utp == null || !utp.getFlag().isReadWrite()) continue;
            freeacs.dbi.UnitParameter up = unit.getUnitParameters().get(utp.getName());
    ...
    private static void importReadOnlyParameters(SessionData sessionData, DBI dbi)
        ...
        List<freeacs.dbi.UnitParameter> unitParameters = new ArrayList<>();
        freeacs.dbi.UnittypeParameters utps = sessionData.getUnittype().getUnittypeParameters();
        for (int i = 0; i < sessionData.getValuesFromCPE().size(); i++) {
            ParameterValueStruct pvsCPE = sessionData.getValuesFromCPE().get(i);
            if (pvsCPE == null || pvsCPE.getValue() == null || pvsCPE.getName() == null) continue;
