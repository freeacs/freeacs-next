...
public class DownloadResponseCreateStrategy implements ResponseCreateStrategy {
    ...
    public Response getResponse(HTTPRequestResponseData reqRes) {
        ...
        freeacs.dbi.util.ProvisioningMessage pm = sessionData.getProvisioningMessage();
        final String downloadType;
        String tn = download.getFile().getTargetName();
        String commandKey = download.getFile().getVersion();
        if (download.getFile().getType() == freeacs.dbi.FileType.SOFTWARE) {
            downloadType = "1 Firmware Upgrade Image";
            pm.setProvOutput(freeacs.dbi.util.ProvisioningMessage.ProvOutput.SOFTWARE);
        } else if (download.getFile().getType() == freeacs.dbi.FileType.TR069_SCRIPT) {
            downloadType = "3 Vendor Configuration File";
            pm.setProvOutput(freeacs.dbi.util.ProvisioningMessage.ProvOutput.SCRIPT);
        } else {
            downloadType = null;
        }
        ...
        String password = sessionData.getAcsParameters().getValue(freeacs.dbi.util.SystemParameters.SECRET);
        Body body = new Body() {
