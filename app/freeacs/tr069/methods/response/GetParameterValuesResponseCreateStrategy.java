...
public class GetParameterValuesResponseCreateStrategy implements ResponseCreateStrategy {
    ...
    public Response getResponse(HTTPRequestResponseData reqRes) {
        ...
        freeacs.dbi.util.ProvisioningMode mode = sessionData.getUnit().getProvisioningMode();
        List<ParameterValueStruct> parameterValueList = new ArrayList<>();
        if (mode == freeacs.dbi.util.ProvisioningMode.READALL) {
            log.debug("Asks for all params (" + sessionData.getKeyRoot() + "), since in " + freeacs.dbi.util.ProvisioningMode.READALL + " mode");
