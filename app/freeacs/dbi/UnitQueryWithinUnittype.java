...
public class UnitQueryWithinUnittype {
  ...
  public static boolean match(
      String fixedOp, String varOp, Operator op, ParameterDataType type) {
    // If fixedOp == null, varOp can match only if null and operator is EQ, or the opposite.
    if (fixedOp == null) {
      return (Operator.EQ.equals(op) && varOp == null)
          || ((!Operator.EQ.equals(op) || varOp == null)
              && (!Operator.NE.equals(op) || varOp != null)
              && Operator.NE.equals(op)
              && varOp != null);
    }
    // If varOp == null, fixedOp can match only if operator is NE (since fixedOp cannot be null).
    if (varOp == null) {
      return Operator.NE.equals(op);
    }
    switch (type) {
      case NUMBER:
        ...
        try {
          ...
          return (Operator.EQ.equals(op) && ppL.longValue() == operandL.longValue())
              || (Operator.NE.equals(op) && ppL.longValue() != operandL.longValue())
              || (Operator.LT.equals(op) && ppL < operandL)
              || (Operator.LE.equals(op) && ppL <= operandL)
              || (Operator.GE.equals(op) && ppL >= operandL)
              || (Operator.GT.equals(op) && ppL > operandL);
      ...
      case TEXT:
        ...
        return (Operator.LT.equals(op) && compareInt < 0)
            || (Operator.LE.equals(op) && compareInt <= 0)
            || (Operator.GE.equals(op) && compareInt >= 0)
            || (Operator.GT.equals(op) && compareInt > 0);
      default:
        return false;
    }
  }
