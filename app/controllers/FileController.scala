...
class FileController(
  ...
  def getFile(
      ...
      unitTypeNameStr: String
  ) = Action {
    val firmwareVersion =
      firmwareVersionStr.replaceAll(DownloadLogicTR069.SPACE_SEPARATOR, " ")
    val unitTypeName =
      unitTypeNameStr.replaceAll(DownloadLogicTR069.SPACE_SEPARATOR, " ")
    ...
    if (unitType == null) {
      ...
      if (firmware == null) {
        logger.error(
          "Could not find " + fileType + " version " + firmwareVersion + " (in unittype " + unitTypeName + ") in xAPS"
        )
        NotFound
      } else {
        val firmwareName = firmware.getName
        logger.debug(
          "Firmware " + firmwareName + " exists, will now retrieve binaries for unittype-name " + unitTypeName
        )
        val firmwareImage = readFirmwareImage(firmware)
        if (firmwareImage == null || firmwareImage.isEmpty) {
          logger.error(
            "No binaries found for firmware " + firmwareName + " (in unittype " + unitTypeName + ")"
          )
          NotFound
        } else {
          Ok(firmwareImage).withHeaders(
  ...
  private def readFirmwareImage(firmwareFresh: File) = {
    ...
    if (firmwareCache != null && Objects.equals(
          firmwareFresh.getId,
          firmwareCache.getId
        )) {
      ...
      baseCache.putFirmware(
        firmwareFresh.getName,
        firmwareFresh.getUnittype.getName,
        firmwareFresh
      )
